...
user = config.altegio_user
bearer = config.altegio_bearer
...
class Record:
    def __init__(self, CRM_data):
        ...
        self.url = 'https://api.altegio.com/api/v1/'
        self.headers = {
    ...
    def get_record(self, record_id='', client_id=None, short_link_token=False):
        ...
        return get_session().get(url=url, headers=self.headers, params=params, timeout=1).json()
    ...
    def get_for_period(self, start_date='', end_date='', client_id=None, short_link_token=True):
        ...
        with get_session().get(url=url, params=params, headers=self.headers, stream=True) as r:
        ...
        if pages > 1:
            for n in range(1, pages + 1):
                ...
                with get_session().get(url, params=params, headers=self.headers, stream=True) as r:
...
class Client:
    ...
    def get_by_id(self, client_id):
        url = self.url + 'client/{}/{}'.format(self.branch, client_id, timeout=1)
    ...
    def get_id_by_phone(self, phone):
        ...
        data = get_session().get(url=url, headers=self.headers, params=params, timeout=1).json()['data']
        if len(data) > 0:
            return data[0]['id']
        else:
            return None
    ...
    def get_info_by_phone(self, phone):
        ...
        data = get_session().get(url=url, headers=self.headers, params=params).json()
        if 'data' in data and len(data['data']) > 0:
            return data['data'][0]
        else:
            return None

    def get_records(self, client_id, start_date='', end_date=''):
        ...
        r = get_session().get(url=url, headers=self.headers, params=params)
        return r.json()
    ...
    def save_clients(self, params):
        ...
        data = get_session().post(url=url, json=params, headers=self.headers, timeout=10).json()

        if ('success' in data and len(data['data']['created']) > 0) or ('created' in data and len(data['created']) > 0):
            return {'success': True}
        else:
...
class Group:
    ...
    def get_client_group_info(self, client_phone):
        ...
        r = get_session().get(url=url, headers=self.headers, params={'phone': client_phone}, timeout=1).json()
        if r['success']:
            return r
        else:
            return False
...
class Loyalty:
    ...
    def get_cards_by_group_id(self, group_id):
        ...
        return get_session().get(url=url, headers=self.headers, timeout=2)
    ...
    def client_have_abonement(self, client_phone):
        ...
        r = requests.get(url=url, headers=self.headers, params=params, timeout=2).json()
...
class Marketplace:
    ...
    def __init__(self, CRM_data):
        ...
        self.url = 'https://api.alteg.io/api/v1/marketplace/partner'
    ...
    def payment(self, payment_sum, period_from, period_to):
        ...
        print('Altegio market payment', result, data, result.text)
        market_info(f'Altegio: {result} {data} {result.text}', 'payment')
        return result.json()

    def refund_payment(self, payment_id):
        url = self.url + f'/refund/{payment_id}'

        result = get_session().post(url=url, headers=self.headers)
        print('Altegio market refund payment', result, result.text)
        market_info(f'Altegio: {result} {result.text}', 'refund')
...
class Services:
    ...
    def get_service_category(self, service_id):
        url = self.url + f'company/{self.branch}/services/{service_id}'

        r = get_session().get(url=url, headers=self.headers)

        assert r.status_code in [200, 201] and r.json()['success'], ' for branch {} service_id {}\n\n{}'.format(
    ...
    def get_all_branch_services(self):
        url = self.url + f'company/{self.branch}/services/'
